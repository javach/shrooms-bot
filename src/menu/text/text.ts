...
Выберите позицию для изменения`,
            ...
            productAdd: (product: Product, cartEntry: CartEntry) =>
                cartEntry
                    ? `Товар <b>${product.name}</b> в количестве <b>${cartEntry.quantity} x ${product.weightPerPackage}</b> уже добавлен в вашу корзину.

Если вы хотите изменить количество товара, отправьте новое в ответном сообщении.`
                    : `Вы добавляете товар <b>${product.name}</b>

Отправьте в отвеном сообщении количество товара (${product.weightPerPackage})`,
